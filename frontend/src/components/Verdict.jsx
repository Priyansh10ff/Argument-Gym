  ...
  return (
    <div className={styles.ring}>
      <svg width="72" height="72" viewBox="0 0 72 72">
        ...
        />
        <text x="36" y="40" textAnchor="middle" fill="white" fontSize="14" fontFamily="'Bebas Neue', sans-serif">{value}</text>
      </svg>
...
export default function Verdict({ verdict, topic, stance, sideSwitch, onReset }) {
  const cardRef = useRef(null);

  const verdictColor = verdict.verdict === 'Won' ? '#4CAF50' : verdict.verdict === 'Lost' ? '#e63329' : '#f5c842';
  const verdictMsg = verdict.verdict === 'Won' ? 'YOU WON' : verdict.verdict === 'Lost' ? 'YOU LOST' : 'DRAW';

  const handleCopy = () => {
    const text = `Argument Gym Results\n\nTopic: ${topic}\nVerdict: ${verdictMsg}\nClarity Score: ${verdict.clarityScore}/100\n\nLogic: ${verdict.scores.logic} | Evidence: ${verdict.scores.evidence} | Originality: ${verdict.scores.originality}${sideSwitch ? ` | Perspective: ${verdict.scores.perspective}` : ''}\n\n"${verdict.overallFeedback}"\n\nargumentgym.app`;
    navigator.clipboard.writeText(text).then(() => alert('Score card copied to clipboard!'));
  };
    ...
    <div className={styles.wrap}>
      <div className={styles.inner}>
        ...
        <div className={styles.ringRow}>
          <ScoreRing value={verdict.scores.logic} label="LOGIC" color="#7F77DD" />
          <ScoreRing value={verdict.scores.evidence} label="EVIDENCE" color="#1D9E75" />
          <ScoreRing value={verdict.scores.originality} label="ORIGINALITY" color="#D85A30" />
          {sideSwitch && <ScoreRing value={verdict.scores.perspective} label="PERSPECTIVE" color="#f5c842" />}
        </div>
